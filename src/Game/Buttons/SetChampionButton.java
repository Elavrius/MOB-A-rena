...
public class SetChampionButton extends PickButton {
	...
	public SetChampionButton(Scene parent, Team team, int nr, String name, int x, int y) {
		super(x, y, 120, 120);
		this.team = team;
		this.name = name;
		number = nr;
	...
	public void use() {
		if (team.getChampion(number) != null) {
			...
			try {
				team.setChampion((Champion) Class.forName("Game.Objects.Champions."+name).newInstance(),
						number);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
