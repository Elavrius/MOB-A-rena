...
public class SetEqButton extends PickButton{
	...
	SetEqButton(Scene parent, Team team, int nr, String name,int socketNr, int x, int y) {
		super(x, y, 120, 120);
	...
	public void use() {
		if (team.getChampion(number) != null) {
			try {
				team.buy((Equipment) Class.forName(name).newInstance(),team.getChampion(number),socketNr);
			} catch (Exception e) {
